 ...
 */
public class InterpolatingDigest extends AbstractTDigest {
    ...
    public InterpolatingDigest(double compression) {
        this(compression, -1);
    }
     ...
     */
    @SuppressWarnings("WeakerAccess")
    public InterpolatingDigest(double compression, int bufferSize) {
     ...
     */
    @SuppressWarnings("WeakerAccess")
    public InterpolatingDigest(double compression, int bufferSize, int size) {
        ix = new Interpolator();
    ...
    public void add(List<? extends TDigest> others) {
        ...
        int offset = 0;
        for (TDigest other : others) {
            if (other instanceof InterpolatingDigest) {
                InterpolatingDigest md = (InterpolatingDigest) other;
    ...
    public double cdf(double x) {
        ...
        if (lastUsedCell == 0) {
            ...
            // note that if weight[0] == 1, we must have min == mean[0]
            if (x < mean[0]) {
                ...
                if (mean[0] - min > 0) {
                    if (x == min) {
                        ...
                        // and that sample can be unwound from the first centroid
                        double xmean = mean[0] + (mean[0] - min) / (weight[0] - 1);
                        double u = (x - min) / (xmean - min);
                        return (1 + ix.cdf(1, weight[0], u) * (weight[0] - 1) / 2) / totalWeight;
                    }
                } else {
                    // this should not be possible except due to very strange numerics
                    return 0;
                }
            }
            assert x >= mean[0];
            ...
            if (x > mean[n - 1]) {
                if (max - mean[n - 1] > 0) {
                    if (x == max) {
                        ...
                        // so we can unwind that sample out of the last mean
                        double xmean = mean[n - 1] - (max - mean[n - 1]) / (weight[n - 1] - 1);
                        double u = (max - x) / (max - xmean);
                        double dq = 1 + interpolate(1, weight[n - 1], u) * ((weight[n - 1] - 1) / 2);
                        return 1 - dq / totalWeight;
                    }
                } else {
                    // this should be impossible except for strange floating point issues
                    return 1;
                }
            }
            ...
            for (int it = 0; it < n - 1; it++) {
                ...
                if (mean[it] == x) {
                    ...
                    if (mean[it + 1] - mean[it] > 0) {
                        ...
                        // interpolation
                        double leftMass = weight[it] > 1 ? weight[it] / 2 : 1;
                        double rightMass = weight[it + 1] > 1 ? weight[it] / 2 : 1;
                        double dw = weight[it] + weight[it + 1];
                        assert leftMass + rightMass <= dw;
                        dw -= leftMass + rightMass;

                        double u = (x - mean[it]) / (mean[it + 1] - mean[it]);
                        double dq = dw * interpolate(weight[it], weight[it + 1], u);
                        return (weightSoFar + leftMass + dq) / totalWeight;
                    } else {
    ...
    public double quantile(double q) {
        ...
        // the min value represents *some* sample so we can take that as a virtual centroid
        if (index < 1) {
            return min;
        }

        // if the left centroid has more than one sample, we still know
        // that one sample occurred at min so we can do some fancy interpolation
        if (weight[0] > 1 && index < weight[0] / 2) {
            assert index >= 1;
            // there is a single sample at min so we interpolate with less weight
            double xweight = weight[0] - 1;
            double xmean = mean[0] + (mean[0] - min) / xweight;
            double u = (index - 1) / (xweight / 2);
            return min + ix.quantile(1, xweight, u) * (xmean - min);
        }
        assert index >= weight[0] / 2;

        // the max has a virtual centroid with unit weight so we can special case that
        ...
        if (weight[n - 1] > 1 && totalWeight - index <= weight[n - 1] / 2) {
            double xweight = weight[n - 1] - 1;
            double xmean = mean[n - 1] + (mean[n - 1] - max) / xweight;
            double u = (totalWeight - index - 1) / (xweight / 2.0);
            return max - ix.quantile(1, xweight, u) * (max - xmean);
        }
        // this assertion is invariant useful at the end of the loop
        assert weight[n-1] == 1 || index < totalWeight - weight[n - 1] / 2;

        // at this point, we have handled everything before with
        // q * totalWeight \not \in (weight[0]/2, 1-weight[n-1]/2)

        // in between extremes we interpolate between centroids
        double weightSoFar = weight[0] / 2;
        for (int i = 0; i < n - 1; i++) {
            assert index >= weightSoFar;
            ...
            double leftExclusion = w1 > 1 ? 0 : 0.5;
            double rightExclusion = w2 > 1 ? 0 : 0.5;

            double dw = (w1 + w2) / 2;
            if (weightSoFar + leftExclusion >= index) {
                // this counts as direct hit on a singleton centroid
                return mean[i];
            }

            if (weightSoFar + dw - rightExclusion >= index) {
                // centroids i and i+1 bracket our current point even after
                // accounting for exclusions
                double u = (index - weightSoFar - leftExclusion) / (dw - leftExclusion - rightExclusion);
                return mean[i] + (mean[i + 1] - mean[i]) * ix.quantile(w1, w2, u);
            } else if (weightSoFar + dw > index) {
                // this is an exact hit on a singleton above us
                return mean[i + 1];
            }
            assert index >= weightSoFar + dw;
            weightSoFar += dw;
        }
        // this applies because of loop structure
        assert weightSoFar >= totalWeight - weight[n - 1] / 2;
        // loop invariant
        assert index >= weightSoFar;
        assert index >= totalWeight - weight[n - 1] / 2;
        assert index < totalWeight;

        // plus the original invariant
        assert weight[n-1] == 1 || index < totalWeight - weight[n - 1] / 2;

        // tells us this
        assert weight[n - 1] == 1;

        // therefore we are inside the zone for a final singleton
        return max;
    }
    ...
    @SuppressWarnings("WeakerAccess")
    public static InterpolatingDigest fromBytes(ByteBuffer buf) {
        ...
        if (encoding == Encoding.VERBOSE_ENCODING.code) {
            ...
            int n = buf.getInt();
            InterpolatingDigest r = new InterpolatingDigest(compression);
            r.setMinMax(min, max);
            r.lastUsedCell = n;
            for (int i = 0; i < n; i++) {
            ...
            InterpolatingDigest r = new InterpolatingDigest(compression, bufferSize, n);
            r.setMinMax(min, max);
