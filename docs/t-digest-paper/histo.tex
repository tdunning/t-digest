...
K(i) + K(i+1) > 1
\]
for all $i=1\ldots m-1$. These conditions imply that the mean value of $K(i)$ is in $(1/2, 1]$ and therefore the number $m$ of such maximally merged groups is bounded according to $ \lfloor\delta\rfloor \le m < 2\left \lceil\delta \right \rceil$. 
...
\For{$(x_n, w_n) \in X$} {
  ...
  $S = \lbrace c_i  :  |c_i.\mathtt{mean} - x| = z \wedge K(c_i+w_1) < 1 \rbrace $\;
...
The number of points assigned to each centroid is limited so no centroid exceeds the $t$-digest size constraint, but pathological insertion orders can cause the number of centroids to increase without bound. Whenever the number of centroids exceeds a heuristically chosen limit, the entire set of centroids is consolidated using the merge algorithm given in Algorithm \ref{alg:merge}.  
