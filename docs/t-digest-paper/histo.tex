...
Alternately, if the bounds on the sizes of the groups of samples is made somewhat tighter, the size of the $t$-digest will instead be $\Omega(\delta \log N)$, but it can be guaranteed when $t$-digests  computed on sub-samples of a dataset can be combined to produce a $t$-digest with the same bounds no matter how the sub-samples are chosen. The currently used sizing strategies that give a digest size independent of $N$ cannot guarantee this for pathological choice of sub-samples, but it is not yet clear if there is a sizing strategy that would allow fixed digest size and parallel computation with guaranteed accuracy. 
...
The $t$-digest has two important formulations. One version keeps an array of samples which are periodically shuffled and merged with an array of centroids, each of which represents the average of one or more samples. This merging version has the virtue of allowing all memory structures to be allocated statically, but it requires more copying of internal data. The other version of the algorithm is more akin to a clustering algorithm in which new samples are selectively added to the nearest centroid.  Both versions are described here and implementations for both are available.

\subsection{Preliminaries}
Suppose first, for argument's sake, that all of the samples $x_1 \ldots x_n$ are presented in sorted order. Take the following as a mapping from quantile $q$ to an index $k$ with compression parameter $\delta$
\[
...
If we group all of the samples $x_1 \ldots x_n$ into sub-sequences of samples $c_i = \lbrace x_{\mathtt {left}(i)} \ldots x_{\mathtt{right}(i)} \rbrace$ specified by the indices $\mathtt{left}(i)$ and $\mathtt{right}(i)$ of the first and last members of the sub-sequences and require that
\[
K(i) = k(\mathtt{right}(i)/n) - k((\mathtt{left}(i)-1)/n) \le 1
\]
If we further assume that no adjacent sub-sequences can be merged without violating this last constraint, then we also know that the mean value of $ K(i)$ is greater than $1/2$. This implies that the number of such maximally merged groups cannot be more than $2 \delta$ nor less than $\delta$. 

In terms of quantile $q$, sample $i$ can be thought of as spanning the range from $q=(i-1)/n$ to $q=i/n$. 
