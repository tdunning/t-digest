...
With the $t$-digest, accuracy for estimating the $q$ quantile is constant relative to $q(1-q)$.  This is in contrast to earlier algorithms which had errors independent of $q$.  The relative error bound of the $t$-digest is convenient when computing quantiles for $q$ near $0$ or $1$ as is commonly required.  As with the Q-digest algorithm, the accuracy/size trade-off for the $t$-digest can be controlled by setting a single compression parameter $\delta$ with the amount of memory required proportional only to $\Omega(\delta)$. 
...
To limit the sub-sequence size, we define a mapping from quantile $q$ to a notional index $k$ with compression parameter $\delta$
\[
...
Figure \ref{fig:linear-interpolation} shows an example of how these improved error bounds arise. In the left panel, the use of equal sized bins means that the interpolated value of $q$ on the left side of the plot has a substantial and unavoidable error. The right panel, on the other hand, shows how the data can be partitioned differently into nearly the same number of bins, but with many fewer points near the beginning or end. The value of $\delta=100$ is quite typical. 

Of course, since we have to put all the samples into some bin, filling some bins with fewer than $100$ samples requires that other bins must have more than $100$ samples. It is to be noted that having fewer samples in the bins near the ends has a large effect on the corresponding errors, but increasing other bin sizes in the middle has a much weaker effect on accuracy. In this particular example, the first and last bins have only 2 samples giving 2500 times smaller error (actually the errors are even smaller since the errors go to zero with only 1 or 2 points) while the central bins are only about $1.6$ times larger, increasing errors by about two and a half times larger relative to equal sized bins. The overall effect is that quantile estimation accuracy is dramatically improved at the extremes but only modestly impaired in the middle of the distribution.
...
The assertion that $t$-digests formed by combination produce accurate quantile estimates has substantial empirical support, but has not been proved rigorously. 

\subsection{The merging algorithm}
The previous section suggests an algorithm for constructing a $t$-digest from a large amount of data. The basic idea is that you collect data in a buffer. When the buffer fills, or when you want a final result, you sort the buffer and combine the buffered data with the existing centroids, merging points or centroids together whenever the size limits can be satisfied by the merged value. With an arbitrarily large buffer, this algorithm reduces to the approach described in the previous section since it would make a single merging pass through all the data. For smaller buffer sizes, however, many merge passes would be done.

The operation of merging a buffer's worth of samples into an existing set of centroids is shown in Algorithm \ref{alg:merge}. Note that, in practice, the check on the size bound is optimized to only require at most $2 \left \lceil \delta \right \rceil$ evaluations of $k(q)$  during the merge. This is done by computing the bounding value $q_{limit} = k^{-1}(k(q) + 1)$ that can be used to limit an inner loop that merges as many values as possible without recomputing $q_{limit}$. If $n \gg 2 \left \lceil \delta \right \rceil$, this can result in a considerable speedup since computing $\sin^{-1}$ is expensive. Note also that all data can be statically allocated as arrays of primitives, avoiding any dynamic allocation or structure boxing/unboxing.
\begin{algorithm}[ht]
...
\KwOut{New ordered set $C'=[c'_1 \ldots c'_m]$ of weighted centroids forming a $t$-digest} 
$X \gets \mathtt{sort}(X)$\;
$C' = \lbrack \, \rbrack$\;
$\sigma = (\mathtt {sum}:0, \mathtt{count}:0), q = 0, s = n + \sum_j c_j.\mathtt{count}$\;
\For{$(x, w) \in \mathtt{merge}(X,C)$} {
  \If {$\sigma.\mathtt{count} = 0 \lor k(q + (\sigma.\mathtt {count}+w)/n) - k(q) \le 1$} {
      $\sigma \gets \sigma + (\mathtt {sum}:x, \mathtt {count}:w)$\;
  } \Else {
      $\mathtt{C'.append}(\sigma)$\;
      $\sigma \gets (\mathtt {sum}:x, \mathtt {count}:w)$\;
  }
} 
\If {$\sigma.\mathtt{count} > 0$} {
     $\mathtt{C'.append}(\sigma)$\;
 }
\Return $ C' $\\
...
The run-time costs of the merge variant of the $t$-digest are a mix of the frequent buffer inserts  and the rare periodic merges. The buffer inserts are very fast since they consist of an array write, index increment and an overflow test. The merges consist of the buffer sort and the merge itself. The merge involves a scan through both the buffer and the existing centroids plus a number of calls to $\sin^{-1}$ roughly equal to the size of the result and thus bounded by $2 \lceil \delta \rceil$. If $k_1$ is the buffer size, the dominant costs are the sort and the $\sin^{-1}$ calls so the amortized input is $C_1 \log k_1 + C_2 2 \lceil \delta \rceil / k_1$ where $C_1$ and $C_2$ are parameters representing the sort and $\sin^{-1}$ costs respectively. This overall cost has a minimum where $k_1 = 2 \lceil \delta \rceil C_2  / C_1$, indicating that  maximum throughput will be achieved when the input buffer size should be proportional to the compression parameter. The constant of proportion should be determined by experiment, but early experiments indicate that $C_2 / C_1$ is in the range from $10$ to $100$ with very substantial degradation in speed below this range. 
